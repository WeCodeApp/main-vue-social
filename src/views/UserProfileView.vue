...
const postFilter = ref<'all' | 'public'>('all')
...
async function fetchProfileData() {
  ...
  try {
    ...
    friends.value = friendsStore.friends.filter(friend => 
      !friendsStore.isBlocked(friend.userId)
    )
...
async function addFriend() {
  ...
  loading.value = true
  try {
    await friendsStore.sendFriendRequest(user.value.uid)
...
function unfriendUser() {
  ...
  confirmTitle.value = `Unfriend ${user.value.displayName || 'User'}`
  confirmMessage.value = `Are you sure you want to remove ${user.value.displayName || 'User'} from your friends? They won't be notified.`
...
function blockUser() {
  ...
  confirmTitle.value = `Block ${user.value.displayName || 'User'}`
  confirmMessage.value = `Are you sure you want to block ${user.value.displayName || 'User'}? They won't be able to see your posts or send you friend requests.`
...
function unblockUser() {
  ...
  confirmTitle.value = `Unblock ${user.value.displayName || 'User'}`
  confirmMessage.value = `Are you sure you want to unblock ${user.value.displayName || 'User'}? They will be able to see your public posts and send you friend requests.`
...
async function confirmActionHandler() {
  ...
  try {
    if (confirmAction.value === 'unfriend') {
      await friendsStore.unfriend(user.value.uid)
      ...
      await friendsStore.unblockUser(user.value.uid)
    }
